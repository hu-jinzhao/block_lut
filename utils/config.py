...
class LUT_MoEConfig:
    ...
    trace_path: str = field(
        default = "/home/hh/LUT-MoE/trace/model_name",
    ...
    lut_path: str = field(
        default = "",
        metadata = {"help": "Path to the LUT table .npy file for LUT codec."}
    )

    lut_tier: int = field(
        default = 0,
        metadata = {"help": "LUT tier for NESTEDLUT: 0=full 256, 1=mapped 64, 2=mapped 16"}
    )

    nested_shallow_layers: int = field(
        default = 8,
        metadata = {"help": "Number of shallow layers using 8-bit (tier 0) in NESTEDLUT static mode."}
    )
