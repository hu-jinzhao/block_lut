...
template<typename PoolType>
class CachePolicyLUT_MoE : public CachePolicy<PoolType> {
private:
...
public:
    ...
    std::string name() const override { return "LUT_MoE"; }
};
...
private:
    ...
    void register_builtin_policies(){
        ...
        register_policy(
            "LUT_MoE",
            [](PoolType* pool){
                return std::make_shared<CachePolicyLUT_MoE<PoolType>>(pool);
            }
        );
        register_policy(
            "LRU",
...
class LUT_MoECacheHandle : public base::noncopyable {

private:
    std::vector<std::unique_ptr<LUT_MoEGPUMemoryPool>>  gpu_pools_;
    std::vector<std::unique_ptr<LUT_MoEPinnedMemoryPool>>  sm_pools_;
    size_t gpu_slot_size_;
    size_t sm_slot_size_;
    std::vector<std::shared_ptr<CachePolicy<LUT_MoEGPUMemoryPool>>> gpu_cache_algorithms_;
    std::vector<std::shared_ptr<CachePolicy<LUT_MoEPinnedMemoryPool>>> sm_cache_algorithms_;
    CachePolicyFactory<LUT_MoEGPUMemoryPool> gpu_policy_factory_;
    CachePolicyFactory<LUT_MoEPinnedMemoryPool> sm_policy_factory_;
...
public:
    ...
    DELETE_COPY_AND_ASSIGN(LUT_MoECacheHandle);
    LUT_MoECacheHandle(
...
extern std::unique_ptr<LUT_MoECacheHandle> kLUT_MoECacheHandle;
