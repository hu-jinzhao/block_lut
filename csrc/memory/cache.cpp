...
std::unique_ptr<LUT_MoECacheHandle> kLUT_MoECacheHandle = nullptr;

LUT_MoECacheHandle::LUT_MoECacheHandle(
...
{
    if (caching_algorithm == "LUT_MoE"){
        use_pool_planning = true;
        DLOG_INFO("[LUT_MoECacheHandle] Applying Cache Pool Planning.");
    }
    ...
    for (size_t pool_id = 0; pool_id < num_pools; pool_id++){
        gpu_pools_.emplace_back(
            std::make_unique<LUT_MoEGPUMemoryPool>(
                size_per_gpu_pool,
                gpu_slot_size,
        ...
        sm_pools_.emplace_back(
            std::make_unique<LUT_MoEPinnedMemoryPool>(
                size_per_sm_pool,
                sm_slot_size,
                pool_id,
                host_handover_ptr,
...
void LUT_MoECacheHandle::allocate_resource(
...
void LUT_MoECacheHandle::SetUpDensePool(
...
void LUT_MoECacheHandle::set_up_cpu_zero_tensors(){
...
void LUT_MoECacheHandle::set_up_global_access_counts(
    int num_layers,
    int num_experts
){
...
void LUT_MoECacheHandle::reset_global_access_counts(){
...
void LUT_MoECacheHandle::assign_layers_to_pool(
    size_t pool_id,
    ...
    if (pool_id >= gpu_pools_.size()||pool_id >= sm_pools_.size()){
        DLOG_FATAL("LUT_MoECacheHandle: Invalid pool_id{}", pool_id);
    }
...
std::vector<std::string> LUT_MoECacheHandle::gpu_available_policies() const {
    std::lock_guard<std::mutex> lock(cache_mutex_);
    return gpu_policy_factory_.available_policies();
}


std::vector<std::string> LUT_MoECacheHandle::sm_available_policies() const {
    std::lock_guard<std::mutex> lock(sm_mutex_);
    return sm_policy_factory_.available_policies();
}


TaskState LUT_MoECacheHandle::ProactiveDecision(int layer_idx, int expert_idx, int num_tokens){
...
void LUT_MoECacheHandle::UpdateOnHit(NodePtr& node, int num_token_reqs){
    ...
    // Dynamic LUT tier promotion based on total visit count
    if (node->is_sparse) {
        int new_tier = 2;  // cold: 4-bit mapped16
        if (node->visit_count >= 50) {
            new_tier = 0;  // hot: 8-bit full256
        } else if (node->visit_count >= 10) {
            new_tier = 1;  // warm: 6-bit mapped64
        }
        new_tier = std::max(new_tier, node->tier_floor);  // don't promote beyond floor
        if (new_tier < node->lut_tier) {
            std::cout << "[LUT_MoE] UpdateOnHit: node " << node->id
                      << " tier " << (int)node->lut_tier << " -> " << new_tier
                      << " (visit_count=" << node->visit_count << ")" << std::endl;
            node->lut_tier = new_tier;  // promote to higher quality (lower number = better)
        }
    }
...
size_t LUT_MoECacheHandle::RegisterCacheSlotForNode(NodePtr& node){
    std::lock_guard<std::mutex> lock(cache_mutex_);
    size_t layer_id = node_to_layer(node);
    size_t pool_id = layer_to_pool(layer_id);
    auto& gpu_pool = gpu_pools_[pool_id];
    ...
    if (gpu_pool->has_free_slot()){
        ...
        if (target_slot_ptr&&target_slot_ptr->node){
            ...
            int old_tier = target_slot_ptr->node->lut_tier;
            // Gradual demotion: 0->1 (8→6 bit), 1→2 (6→4 bit), 2 stays at 2
            int new_tier = (old_tier == 0) ? 1 : 2;
            new_tier = std::min(new_tier, target_slot_ptr->node->tier_ceiling);
            std::cout << "[LUT_MoE] Evict: node " << target_slot_ptr->node->id
                      << " tier " << old_tier << " -> " << new_tier
                      << (new_tier == 1 ? " (warm)" : " (cold)") << std::endl;
            target_slot_ptr->node->lut_tier = new_tier;
...
CacheSlot* LUT_MoECacheHandle::GetCacheSlotSlot(NodePtr& node){
    std::lock_guard<std::mutex> lock(cache_mutex_);
    size_t layer_id = node_to_layer(node);
    size_t pool_id = layer_to_pool(layer_id);
    auto& gpu_pool = gpu_pools_[pool_id];
...
void LUT_MoECacheHandle::UnregisterCacheSlotForNode(NodePtr& node){
    std::lock_guard<std::mutex> lock(cache_mutex_);
    size_t layer_id = node_to_layer(node);
    size_t pool_id = layer_to_pool(layer_id);
    auto& gpu_pool = gpu_pools_[pool_id];
...
bool LUT_MoECacheHandle::IsSMCached(NodePtr& node){
    std::lock_guard<std::mutex> lock(sm_mutex_);
    size_t layer_id = node_to_layer(node);
    size_t pool_id = layer_to_pool(layer_id);
    auto& sm_pool = sm_pools_[pool_id];
...
size_t LUT_MoECacheHandle::RegisterSMCacheForNode(NodePtr& node){
    std::lock_guard<std::mutex> lock(sm_mutex_);
    size_t layer_id = node_to_layer(node);
    size_t pool_id = layer_to_pool(layer_id);
    auto& sm_pool = sm_pools_[pool_id];
...
SMSlot* LUT_MoECacheHandle::GetSMCacheSlot(NodePtr& node){
    std::lock_guard<std::mutex> lock(sm_mutex_);
    size_t layer_id = node_to_layer(node);
    size_t pool_id = layer_to_pool(layer_id);
    auto& sm_pool = sm_pools_[pool_id];
...
void LUT_MoECacheHandle::UnregisterSMCacheForNode(NodePtr& node){
    std::lock_guard<std::mutex> lock(sm_mutex_);
    size_t layer_id = node_to_layer(node);
    size_t pool_id = layer_to_pool(layer_id);
    auto& sm_pool = sm_pools_[pool_id];
...
template class CachePolicyLUT_MoE<LUT_MoEGPUMemoryPool>;
template class CachePolicyLUT_MoE<LUT_MoEPinnedMemoryPool>;

template class CachePolicyLRU<LUT_MoEGPUMemoryPool>;
template class CachePolicyLRU<LUT_MoEPinnedMemoryPool>;

template class CachePolicyLFU<LUT_MoEGPUMemoryPool>;
template class CachePolicyLFU<LUT_MoEPinnedMemoryPool>;


template class CachePolicyMarking<LUT_MoEGPUMemoryPool>;
template class CachePolicyMarking<LUT_MoEPinnedMemoryPool>;


template class CachePolicyFIFO<LUT_MoEGPUMemoryPool>;
template class CachePolicyFIFO<LUT_MoEPinnedMemoryPool>;


template<typename PoolType>
size_t CachePolicyLUT_MoE<PoolType>::select_victim_slot(){
    ...
    if (victim_slot == SIZE_MAX){
        DLOG_FATAL("[LUT_MoE]: No victim found! All slots are either free or protected!");
    }
    return victim_slot;
}
...
template<typename PoolType>
void CachePolicyLUT_MoE<PoolType>::on_hit(size_t slot_id, size_t num_tokens){
...
template<typename PoolType>
void CachePolicyLUT_MoE<PoolType>::on_miss(size_t slot_id, uint32_t layer_id){
    if (slot_id < num_slots_ ){
...
template<typename PoolType>
void CachePolicyLUT_MoE<PoolType>::on_evict(size_t slot_id){
    if (slot_id < num_slots_){
