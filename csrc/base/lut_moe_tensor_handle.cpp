...
#include "lut_moe_tensor_handle.hpp"
...
const char* LUT_MOE_PARAM_NAME = "lut_moe_param";
const char* LUT_MOE_INDEX_NAME = "lut_moe_index";

std::unique_ptr<LUT_MoETensorHandle> kLUT_MoETensorHandle(nullptr);

LUT_MoETensorHandle::LUT_MoETensorHandle(const std::string& prefix)
...
void LUT_MoETensorHandle::StoreTensor(
    ...
    tensor_meta.num_file_chunks = kLUT_MoETensorEngine->config_ptr->num_file_chunks;
    tensor_meta.exp_file_offsets = std::vector<size_t>(kLUT_MoETensorEngine->config_ptr->num_file_chunks);
    tensor_meta.compressed_sizes = std::vector<size_t>(kLUT_MoETensorEngine->config_ptr->num_file_chunks);
    tensor_meta.original_sizes = std::vector<size_t>(kLUT_MoETensorEngine->config_ptr->num_file_chunks);
    tensor_meta.sm_file_offset = 0;
    tensor_meta.sm_size = 0;
    kTensorIndex->insert({tensor_id, tensor_meta});
    lock.unlock();
    if (is_sparse){
        kLUT_MoETensorEngine->PipelineCompress(
            exponents_chunks,
            sign_mantissa,
            tensor_id
        );
    } else {
        kLUT_MoETensorEngine->OffloadDense(
            buffer,
            tensor_id
        );
    }
...
void LUT_MoETensorHandle::BatchStoreTensor(
    ...
    kLUT_MoETensorEngine->BatchPipelineCompress(
        tensor_ids,
...
int64_t LUT_MoETensorHandle::GetTensorSizeAligned(
...
torch::TensorOptions LUT_MoETensorHandle::GetTensorOptions(
    const uint32_t tensor_id
) const {
    auto it = kTensorIndex->find(tensor_id);
    if (it == kTensorIndex->end()) {
...
void LUT_MoETensorHandle::SetTensor(
    uint32_t tensor_id,
    torch::Tensor& buffer,
...
void LUT_MoETensorHandle::RegisterTensor(
    const uint32_t tensor_id,
    torch::Tensor& buffer
){
    auto it = kTensorIndex->find(tensor_id);
...
std::string LUT_MoETensorHandle::GetIndexFileName(
        const uint32_t file_id
) const {
    return prefix_ + std::string(LUT_MOE_PARAM_NAME) + "_" + std::to_string(file_id);
}


uint32_t LUT_MoETensorHandle::GetTensorId(void* tensor) const {
...
void LUT_MoETensorHandle::UpdateTensorMap(void* old_data_ptr,
                                         void* new_data_ptr) {
