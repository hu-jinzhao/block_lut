...
std::unique_ptr<LUT_MoETaskPool> kTaskPool = nullptr;

LUT_MoETaskPool::LUT_MoETaskPool(){
    ...
    for (int i = 0; i < num_gpu; ++i){
        ...
        for (int j = 0; j < num_thread_per_gpu; ++j ){
            auto gpu_thread = std::thread(&LUT_MoETaskPool::GPUThreadFunc, this, i, j);
...
void LUT_MoETaskPool::GPUThreadFunc(int gpu_id, int thread_id) {
    while (true) {
...
void LUT_MoETaskPool::SetNodeDevice(const TaskPtr& task){
    auto node = task->node;
...
void LUT_MoETaskPool::EnqueueTask( const TaskPtr& task ){
...
void LUT_MoETaskPool::FetchExec(
...
void LUT_MoETaskPool::StartExec(
    const uint64_t& request_id,
    const NodePtr& node
){
    auto task = std::make_shared<Task>();
