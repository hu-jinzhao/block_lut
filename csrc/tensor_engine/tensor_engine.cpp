...
std::unique_ptr<TensorEngine> kLUT_MoETensorEngine = nullptr;
...
LUT_MoECompressor TensorEngine::initialize_compressor(
    ...
    else if ( CODE_TYPE == "RAW" || CODE_TYPE == "LUT" ){
        // RAW: no compression, raw bf16 exponent+sign-mantissa (lossless baseline)
        // LUT: 8-bit indices stored as-is
        if (CODE_TYPE == "LUT") config_ptr->is_lut = true;
        return std::make_unique<IdentityCompressor>();
    }
    else if ( CODE_TYPE == "BLOCKLUT" || CODE_TYPE == "NESTEDLUT" ){
        config_ptr->is_blocklut = true;
        return std::make_unique<IdentityCompressor>();
    }
    else {
        DLOG_FATAL("[LUT_MoE] Unknown Compressor Type !");
        return std::make_unique<LZ4HCcompressor>(config_ptr->LZ4HC_compressionLevel);
    }
}
...
TensorEngine::~TensorEngine(){
    ...
    if (lut_device_ != nullptr) {
        cudaFree(lut_device_);
        lut_device_ = nullptr;
    }
    if (lut_device_mapped64_ != nullptr) {
        cudaFree(lut_device_mapped64_);
        lut_device_mapped64_ = nullptr;
    }
    if (lut_device_mapped16_ != nullptr) {
        cudaFree(lut_device_mapped16_);
        lut_device_mapped16_ = nullptr;
    }
    cudaStreamDestroy(tensor_stream);
}


void TensorEngine::SetLUTTable(py::array_t<uint16_t>& lut_array){
    py::buffer_info buf = lut_array.request();
    if (buf.size != 256){
        DLOG_FATAL("[LUT_MoE] LUT table must have exactly 256 entries!");
        return;
    }
    lut_table_.resize(256);
    std::memcpy(lut_table_.data(), buf.ptr, 256 * sizeof(uint16_t));

    if (lut_device_ != nullptr){
        cudaFree(lut_device_);
    }
    cudaMalloc(&lut_device_, 256 * sizeof(uint16_t));
    cudaMemcpy(lut_device_, lut_table_.data(), 256 * sizeof(uint16_t), cudaMemcpyHostToDevice);
    DLOG_INFO("[LUT_MoE] LUT table uploaded to GPU (256 x uint16)");
}


void TensorEngine::SetNestedLUTTables(
    py::array_t<uint16_t>& lut_full,
    py::array_t<uint16_t>& lut_mapped64,
    py::array_t<uint16_t>& lut_mapped16
){
    auto upload_one = [](py::array_t<uint16_t>& arr, uint16_t*& dev_ptr, const std::string& name) {
        py::buffer_info buf = arr.request();
        if (buf.size != 256){
            std::cerr << "[LUT_MoE] Nested LUT " << name << " must have exactly 256 entries! Got " << buf.size << std::endl;
            return;
        }
        if (dev_ptr != nullptr) cudaFree(dev_ptr);
        cudaMalloc(&dev_ptr, 256 * sizeof(uint16_t));
        cudaMemcpy(dev_ptr, buf.ptr, 256 * sizeof(uint16_t), cudaMemcpyHostToDevice);
        std::cout << "[LUT_MoE] Nested LUT " << name << " uploaded to GPU" << std::endl;
    };

    upload_one(lut_full, lut_device_, "full(256)");
    upload_one(lut_mapped64, lut_device_mapped64_, "mapped64");
    upload_one(lut_mapped16, lut_device_mapped16_, "mapped16");
    nested_lut_active_ = true;
    DLOG_INFO("[LUT_MoE] Nested LUT mode activated: 3 tiers (256/64/16)");
}
...
size_t TensorEngine::write_compressed_append(
    ...
    if(!file.is_open()){
        DLOG_FATAL("[LUT_MoE] Could not open file for appending: ", config_ptr->offload_file);
    }
...
void TensorEngine::PipelineCompress(
    ...
    size_t sm_size = static_cast<size_t>(sm_buf.size);
    {
        ...
        DLOG_TRACE("[LUT_MoE] Compression completes for tensor: ", tensor_id);
    }
}
...
void TensorEngine::BatchPipelineCompress(
...

    {
        ...
        for (size_t i = 0; i < batch_size; ++i) {
            ...
            meta.sm_file_offset = sm_offset;
            DLOG_TRACE("[LUT_MoE] Batch Offload: Finished tensor ", tid);
        }
    }
}
...
void TensorEngine::OffloadDense(
    ...
    DLOG_TRACE("[LUT_MoE] Dense offload completes for tensor: ", tensor_id);
}
...
void TensorEngine::OpenOffloadFile(){
    ...
    if (fd_offload_file == -1){
        DLOG_FATAL("[LUT_MoE] Offload File Does NOT Exist!");
    }

    DLOG_INFO("[LUT_MoE] Opened offloaded file: ", offload_file_path);
}
...
std::future<void> TensorEngine::io_sign_mantissa_submit(
    ...
    auto [operation, future] = create_task<void>(
        task->priority,
        "",
        [this, task, sm_file_offset, host_sm_pinned_ptr, sm_size]
        (std::shared_ptr<PreemptToken> token, int thread_idx){
            ...
            try{
                ...
                if (n == -1){
                    DLOG_INFO("[LUT_MoE Error] pread SM failed! ");
                    std::cerr << "[LUT_MoE Error] pread SM failed! Errno: " << errno << std::endl;
                }
                nvtxRangePop();

            } catch(...) {
                std::cerr << "[LUT_MoE Critical] Exception inside SM IO thread" << std::endl;
            }
    ...
    auto [operation, future] = create_task<void>(
        ...
        (std::shared_ptr<PreemptToken> token, int thread_idx){
            ...
            try {
                ...
                nvtxRangePop();
                // Progressive read: cold/warm read only portion of packed data
                size_t read_size = compressed_size;
                if (config_ptr->is_blocklut && task->node->lut_tier > 0) {
                    int tier = task->node->lut_tier;
                    if (tier == 2) read_size = original_size / 2;       // 4-bit: read low section only
                    else if (tier == 1) read_size = original_size * 3 / 4;  // 6-bit: read low + mid
                }
                ...
                if (n == -1){
                    DLOG_INFO("[LUT_MoE Error] pread failed in worker thread! ");
                    std::cerr << "[LUT_MoE Error] pread failed in worker thread! Errno: " << errno << std::endl;
                    throw std::runtime_error("[LUT_MoE] failed to pread chunk !");
                }
                nvtxRangePop();
                ...
                // CPU unpack for 6-bit warm tier (4-bit cold unpack is done on GPU)
                if (config_ptr->is_blocklut && task->node->lut_tier == 1) {
                    size_t n = original_size;
                    size_t half = n / 2, quart = n / 4;
                    for (size_t i = 0; i < quart; i++) {
                        uint8_t mid_byte = host_exp_pinned_ptr_with_offset[half + i];
                        for (int j = 0; j < 4; j++) {
                            size_t elem = 4*i + j;
                            uint8_t low_byte = host_exp_pinned_ptr_with_offset[2*i + j/2];
                            uint8_t low = (j%2==0) ? (low_byte & 0x0F) : ((low_byte>>4) & 0x0F);
                            host_exp_pinned_ptr_with_offset[elem] = low | (((mid_byte>>(2*j)) & 0x03) << 4);
                        }
                    }
                }
                nvtxRangePop();
            } catch (...) {
                std::cerr << "[LUT_MoE Critical] Exception inside worker thread for Task " << task->layer_idx << std::endl;
            }
            if (task->on_operation_done()){
                task_ready_notify(task);
            };
...
void TensorEngine::task_ready_notify(const LUT_MoETaskPtr& task){
...
void TensorEngine::unregister_memory(LUT_MoETaskPtr& task){
    kMemoryDock->UnregisterDockForNode(task->node);
    switch (task->state){
        case TaskState::EVICT:
            break;
        case TaskState::SMCACHE:
            kLUT_MoECacheHandle->UnregisterSMCacheForNode(task->node);
            break;
        case TaskState::NORMAL:
        case TaskState::PREFETCH:
            kLUT_MoECacheHandle->UnregisterCacheSlotForNode(task->node);
            break;
        default:
            assert(false);
    }
...
void TensorEngine::ToWorkers(LUT_MoETaskPtr& task){
    {
    ...
    switch (task->state){
        ...
        case TaskState::SMCACHE:{
            kLUT_MoECacheHandle->RegisterSMCacheForNode(task->node);
            auto sm_cache = kLUT_MoECacheHandle->GetSMCacheSlot(task->node);
            task->receive_ptrs(
                dock->get_tensor(),
                dock->get_exp_pinned(0).second,
                sm_cache->ptr.second
        ...
        case TaskState::PREFETCH:{
            ...
            kLUT_MoECacheHandle->RegisterCacheSlotForNode(task->node);
            auto gpu_cache = kLUT_MoECacheHandle->GetCacheSlotSlot(task->node);
            task->receive_ptrs(
                gpu_cache->ptr,
                dock->get_exp_pinned(0).second,
                device_sm_pinned_ptr
    ...
    for (int tensor_idx = 0; tensor_idx < task->node->tensor_ids.size(); tensor_idx++){
        ...
        if ( needs_sm_io && !config_ptr->is_lut ){
...
void TensorEngine::LUT_MoESchedulerFunc(){
    {
        std::lock_guard<std::mutex> lock(scheduled_queue_mutex);
    ...
    std::priority_queue<
        LUT_MoETaskPtr, std::vector<LUT_MoETaskPtr>, LUT_MoETaskPtrCompare
    > TypeI_pq;
    std::priority_queue<
        LUT_MoETaskPtr, std::vector<LUT_MoETaskPtr>, LUT_MoETaskPtrCompare
    > TypeII_pq;
