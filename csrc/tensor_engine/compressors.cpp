...
// --- IdentityCompressor (no-op passthrough for LUT indices) ---

std::unique_ptr<uint8_t[]> IdentityCompressor::get_compressed_buffer(
    const size_t& input_size,
    size_t& compress_bound
){
    compress_bound = input_size;  // no expansion
    return std::make_unique<uint8_t[]>(input_size);
}

void IdentityCompressor::compress(
    ...
    compress_bound = uncompressed_size;
    std::memcpy(compressed_ptr, uncompressed_ptr, uncompressed_size);
    compressed_size = uncompressed_size;
}

std::unique_ptr<uint8_t[]> IdentityCompressor::get_decompressed_buffer(
    const size_t& original_size
){
    return std::make_unique<uint8_t[]>(original_size);
}

void IdentityCompressor::decompress(
    uint8_t* compressed_ptr,
    size_t& compressed_size,
    size_t& original_size,
    uint8_t* decompressed_ptr
){
    std::memcpy(decompressed_ptr, compressed_ptr, compressed_size);
}
