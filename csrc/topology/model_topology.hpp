...
struct Node{
    ...
    int lut_tier = 0;  // 0=full 256, 1=mapped 64, 2=mapped 16 — start hot, demote on eviction
    int tier_floor = 0;  // minimum tier (best quality allowed), clamp promotion
    int tier_ceiling = 2;  // maximum tier (worst quality allowed), clamp demotion
    int last_decompress_tier = -1;  // track tier used for last decompression
public:
    explicit Node();
...
class LUT_MoETopologyHandle: public base::noncopyable{
public:
    DELETE_COPY_AND_ASSIGN(LUT_MoETopologyHandle);
    LUT_MoETopologyHandle();
    ~LUT_MoETopologyHandle() = default;
...
extern std::unique_ptr<LUT_MoETopologyHandle> kTopologyHandle;
