...
torch::Tensor ExpertDispatcher::ObtainTensor(
    ...
    CacheSlot* slot = kLUT_MoECacheHandle->GetCacheSlotSlot(node);
...
void ExpertDispatcher::EnqueueExpert(
    ...
    TaskState task_state = kLUT_MoECacheHandle->ProactiveDecision(layer_idx, expert_idx, num_token_reqs);
    num_enqueued_.fetch_add(1);

    {
        std::lock_guard<std::mutex> lock(kLUT_MoETensorEngine->scheduled_queue_mutex);
        for ( auto& existing_task: kLUT_MoETensorEngine->scheduled_task_queue ){
            if (
                ...
                kLUT_MoECacheHandle->UpdateOnHit(existing_task->node, num_token_reqs);
                return;
            }
        }
    }
    {
        std::lock_guard<std::mutex> lock(kLUT_MoETensorEngine->ready_queue_mutex);
        for ( auto& ready_task: kLUT_MoETensorEngine->ready_queue ){
            if (
                ...
                kLUT_MoECacheHandle->UpdateOnHit(ready_task->node, num_token_reqs);
                return; 
            }
        }
    }
    if (expert_node->node->device.is_cuda()){
        kLUT_MoECacheHandle->UpdateOnHit(expert_node->node, num_token_reqs);
    }
    expert_node->node->mutex.lock();
    std::lock_guard<std::mutex> lock(kLUT_MoETensorEngine->new_queue_mutex);
    LUT_MoETaskPtr task = std::make_shared<LUT_MoETask>();
    task->priority = 0.0;
    ...
    int total_chunks = kLUT_MoETensorEngine->config_ptr->num_file_chunks * expert_node->node->tensor_ids.size();
    ...
    if (task->node->device.is_cuda()){
        {
            ...
            kLUT_MoETensorEngine->ready_queue.push_back(std::move(task));
        }
        kLUT_MoETensorEngine->ready_queue_cv.notify_all();
    } else {
        kLUT_MoETensorEngine->new_task_queue.insert(std::move(task));
    }
...
void ExpertDispatcher::SubmitPrefetch(
    ...
    if (task->node->device.is_cuda()){
        ...
        kLUT_MoETensorEngine->ToWorkers(task);
    }
}


void ExpertDispatcher::OperationSchedule(){
    kLUT_MoETensorEngine->LUT_MoESchedulerFunc();
}
...
void ExpertDispatcher::GPUFetchFunc(int gpu_id){
    ...
    while (!main_thread_stop_flag_.load()){
        std::unique_lock<std::mutex> lock(kLUT_MoETensorEngine->ready_queue_mutex);
        kLUT_MoETensorEngine->ready_queue_cv.wait(
            lock, 
            [&]{
                bool not_empty = !kLUT_MoETensorEngine->ready_queue.empty();
                return not_empty;
            }
        );
        LUT_MoETaskPtr task = std::move( kLUT_MoETensorEngine->ready_queue.front() );
        kLUT_MoETensorEngine->ready_queue.pop_front();
        ...
        task->node->SetDevice(
            ...
            task->sm_ptr,
            task->tensor_ptr,
            kLUT_MoETensorEngine->tensor_stream
        );
...
void ExpertDispatcher::GPUExecFunc(int gpu_id){
    ...
    while ( !main_thread_stop_flag_.load() ){
        ...
        {
            // Inline output processing to skip OutputThreadFunc stage
            // OutputFunc logic (from OutputThreadFunc):
            auto output_device =
            ...
            switch (args.task_state){
                ...
                case TaskState::EVICT:
                    kLUT_MoECacheHandle->UnregisterCacheSlotForNode(args.expert_node->node);
                    kLUT_MoECacheHandle->UnregisterSMCacheForNode(args.expert_node->node);
                    break;
            }
