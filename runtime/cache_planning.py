...
def CachePoolPlanningSimu(
    ...
    for ratio_full in ratio_full_cache_list:
        ...
        print(f"[LUT_MoE Invocation Modeling] Ratio Full: {ratio_full:.2f} | Avg Delay: {avgDelay:.4f}")
    ...
    for i in range(len(smooth_simu_delay_list)):
        print(f"[LUT_MoE Cost Modeling]: Smoothed Ratio Full = {ratio_list[i]},  Cost = {smooth_simu_delay_list[i]}")
...
def ObtainDistFromTrace(
    ...
    print("[LUT_MoE Cache Pool Planning] Loading trace...")
...
def CoverageToWeight(
    ...
    if not np.isclose(np.sum(pi), k):
        raise ValueError(f"[LUT_MoE Trace Error] Sum of target probabilities ({np.sum(pi)}) must equal k: ({k})")
    # Initialize w
    w = pi / (1.0 - pi + 1e-12) 
...
def PlanCache(
    ...
    print(f"[LUT_MoE Cache Pool Planning] Best ratio: {best_cache_pool_ratio:.2f}")
