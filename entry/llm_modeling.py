...
from utils import LUT_MoEConfig, get_checkpoint_paths
from runtime import LUT_MoEEngine
...
class MoE:
    def __init__(
        ...
        if isinstance(config, dict):
            engine_config = LUT_MoEConfig.load_from_json(config)
        else:
            engine_config = LUT_MoEConfig.load_from_file(config)

        self.engine = LUT_MoEEngine(
            model_config
        )
    ...
    def generate(self, input_ids: torch.LongTensor, **kwargs) -> Any:
        ...
        self.engine.lut_moe_engine.reset_access_counts()
